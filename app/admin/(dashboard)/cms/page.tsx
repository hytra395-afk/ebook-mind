          ...
          <tbody>
            {landingPages?.map((page: any) => (
              <tr key={page.id} className="border-b last:border-0 hover:bg-gray-50">
                ...
                <td className="px-6 py-4 text-sm text-gray-600">{page.hero_title || '-'}</td>
                <td className="px-6 py-4">
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    page.active ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-500'
                  }`}>
                    {page.active ? 'Active' : 'Draft'}
                  </span>
                </td>
                <td className="px-6 py-4 flex items-center gap-3">
                  <Link href={`/admin/cms/${page.id}`} className="text-purple-600 hover:text-purple-800">
